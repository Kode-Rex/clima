...
class WeatherSSEManager:
    ...
    async def _zip_to_location_key(self, zip_code: str) -> tuple[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Converted zip code {zip_code} to location: {location_name} (Key: {location_key})"
            )
    ...
    def add_connection(
        ...
        logger.info(
            f"Added SSE connection {connection_id} for {location_name or zip_code} (location key: {location_key})"
        )
        return connection
    ...
    async def _heartbeat_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.connections:
                    logger.debug(
                        f"Heartbeat check: {len(self.connections)} active connections"
                    )
            ...
            try:
                ...
                for location_key in self.location_subscribers.keys():
                    try:
                        ...
                        logger.error(
                            f"Error checking alerts for location {location_key}: {e}"
                        )
    ...
    async def _broadcast_alerts(self, location_key: str, alerts: List[WeatherAlert]):
        ...
        for connection_id in subscriber_ids:
            ...
            for alert in alerts:
                if (
                    "all" in connection.alert_types
                    or alert.category.lower() in connection.alert_types
                ):
            ...
            if filtered_alerts:
                ...
                logger.info(
                    f"Broadcasting {len(filtered_alerts)} alerts to connection {connection_id}"
                )
    ...
    async def get_current_weather_stream(self, location_key: str, connection_id: str):
        ...
        async def event_generator():
            ...
            try:
                ...
                try:
                    ...
                    if current_alerts:
                        ...
                        if filtered_alerts:
                            ...
                            logger.info(
                                f"Sent {len(filtered_alerts)} immediate alerts to connection {connection_id}"
                            )
                ...
                logger.error(
                    f"Error sending initial weather data to {connection_id}: {e}"
                )
            ...
            while self.running and connection_id in self.connections:
                try:
                    ...
                    if (current_time - last_alert_check).total_seconds() >= 60:
                        try:
                            current_alerts = (
                                await self.weather_client.get_weather_alerts(
                                    location_key
                                )
                            )
                            cached_alerts = self.alert_cache.get(location_key, [])
                            ...
                            new_alerts = []
                            cached_alert_ids = {
                                alert.alert_id for alert in cached_alerts
                            }
                            ...
                            if new_alerts:
                                ...
                                for alert in new_alerts:
                                    if (
                                        "all" in connection.alert_types
                                        or alert.category.lower()
                                        in connection.alert_types
                                    ):
                                        filtered_alerts.append(alert)
                                ...
                                if filtered_alerts:
                                    ...
                                    logger.info(
                                        f"Sent {len(filtered_alerts)} new alerts to connection {connection_id}"
                                    )
                    ...
                    if (current_time - last_weather_update).total_seconds() >= 120:
                        ...
                        logger.debug(
                            f"Sent weather update to connection {connection_id}"
                        )
...
class WeatherSSEApp:
    ...
    def _setup_routes(self):
        ...
        async def weather_stream(
            zip_code: str, request: Request, alert_types: str = "all"
        ):
            ...
            try:
                ...
                # Parse alert types
                alert_type_set = (
                    set(alert_types.split(",")) if alert_types != "all" else {"all"}
                )
                ...
                event_generator = await self.sse_manager.get_current_weather_stream(
                    location_key, connection_id
                )
