...
def setup_mcp_tools(mcp, weather_client: "NationalWeatherServiceClient"):
    ...
    async def get_current_weather(location: LocationKey) -> dict:
        ...
        return await weather_service.get_current_weather(
            location.location_key, location.details
        )

    @mcp.tool()
    async def get_5day_forecast(forecast: ForecastQuery) -> dict:
        """Get 5-day weather forecast for a location"""
        return await forecast_service.get_5day_forecast(
            forecast.location_key, forecast.metric
        )
    ...
    async def get_7day_forecast(forecast: ForecastQuery) -> dict:
        ...
        return await forecast_service.get_extended_forecast(
            forecast.location_key, days=7, metric=forecast.metric
        )
    ...
    async def get_extended_forecast(forecast: ExtendedForecastQuery) -> dict:
        ...
        return await forecast_service.get_extended_forecast(
            forecast.location_key, forecast.days, forecast.metric
        )
    ...
    async def get_hourly_forecast(forecast: HourlyForecastQuery) -> dict:
        ...
        return await forecast_service.get_hourly_forecast(
            forecast.location_key, forecast.hours, forecast.metric
        )
    ...
    async def get_detailed_grid_data(location: LocationKey) -> dict:
        ...
        return await raw_weather_service.get_detailed_grid_data(
            location.location_key, metric=True
        )

    @mcp.tool()
    async def get_location_extended_forecast(query: LocationQuery) -> dict:
        """Get 7-day forecast by searching for a location first"""
        return await location_service.get_location_extended_forecast(
            query.query, days=7, language=query.language
        )

    @mcp.tool()
    async def get_location_hourly_forecast(query: LocationQuery) -> dict:
        """Get hourly forecast by searching for a location first"""
        return await location_service.get_location_hourly_forecast(
            query.query, hours=168, language=query.language
        )
