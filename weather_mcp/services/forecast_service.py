...
class ForecastService:
    ...
    async def get_5day_forecast(self, location_key: str, metric: bool = True) -> dict:
        ...
        try:
            forecasts = await self.weather_client.get_5day_forecast(location_key, metric)
            return {
                "success": True,
                "forecasts": [
                    {
    ...
    async def get_extended_forecast(self, location_key: str, days: int = 7, metric: bool = True) -> dict:
        """Get extended weather forecast for a location (up to 7 days)"""
        try:
            forecasts = await self.weather_client.get_daily_forecast(location_key, days, metric)
            return {
                "success": True,
                "forecasts": [
                    {
    ...
    async def get_hourly_forecast(self, location_key: str, hours: int = 168, metric: bool = True) -> dict:
        """Get hourly weather forecast for a location (up to 168 hours/7 days)"""
        try:
            forecasts = await self.weather_client.get_hourly_forecast(location_key, hours, metric)
            return {
                "success": True,
                "forecasts": [
                    {
