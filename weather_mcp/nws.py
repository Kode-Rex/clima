        ...
        try:
            ...
            for station in stations[:3]:  # Try first 3 stations
                try:
                    ...
                    pressure_pa = properties.get("barometricPressure", {}).get("value", 0)
                    ...
                    precip_m = properties.get("precipitationLastHour", {}).get("value", 0)
                    precip_in = precip_m * 39.3701 if precip_m else 0
                    
                    # Get observation time
                    obs_time_str = properties.get("timestamp", datetime.now().isoformat())
                    ...
                    logger.debug(f"Station {station.get('properties', {}).get('stationIdentifier', 'unknown')} failed: {station_error}")
                    continue
    ...
    async def get_5day_forecast(self, location_key: str, metric: bool = True) -> List[WeatherForecast]:
        ...
        try:
            ...
            for period in periods:
                date = datetime.fromisoformat(period["startTime"].replace("Z", "+00:00")).date()
            ...
            for date, day_data in sorted(day_periods.items()):
                ...
                night_temp = night_period.get("temperature", 0) if night_period else day_temp - 10
                ...
                forecast = WeatherForecast(
                    ...
                    day_weather_icon=self._text_to_icon(day_period.get("shortForecast", "")),
                    day_precipitation_probability=day_period.get("probabilityOfPrecipitation", {}).get("value", 0) or 0,
                    night_weather_text=night_period.get("shortForecast", "") if night_period else "",
                    night_weather_icon=self._text_to_icon(night_period.get("shortForecast", "")) if night_period else 0,
                    night_precipitation_probability=night_period.get("probabilityOfPrecipitation", {}).get("value", 0) or 0 if night_period else 0
                )
    ...
    async def get_weather_alerts(self, location_key: str) -> List[WeatherAlert]:
        ...
        try:
            ...
            for feature in data.get("features", []):
                ...
                # Parse times
                start_time = datetime.fromisoformat(properties.get("onset", "").replace("Z", "+00:00"))
                end_time = None
                if properties.get("expires"):
                    end_time = datetime.fromisoformat(properties.get("expires", "").replace("Z", "+00:00"))
